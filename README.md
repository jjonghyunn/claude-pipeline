...
각 에이전트는 **git worktree** 안에서 격리된 서브프로세스로 실행되어, 메인 브랜치를 건드리지 않습니다. `.pipeline/` 디렉토리의 마크다운 파일로 소통합니다.
...
`/pipeline` 실행 시 전용 git worktree와 브랜치가 생성되어, 메인 브랜치는 머지 전까지 깨끗하게 유지됩니다.
...
| `/sanicheck` | 커밋 전 새니타이즈 룰 위반 스캔 및 수정 | O |
| `/makemd` | Python 스크립트 읽어 한국어 md 문서 자동 생성 | O |
| `/new-campaign` | 새 캠페인 로컬 폴더 구조 생성 | O |
